...
parser Layer4Parser(
    ...
    state parse_ipv4 {
        ...
        transition select(hdr.ipv4.protocol) {
            ...
            IP_PROTOCOLS_TELEMETRY : parse_int;
            default : accept;
        }
    }
    state parse_int {
...
parser SwitchEgressParser(
    ...
    state parse_mirror_md {
        pkt.extract(eg_md.mirror_header);
        layer4_parser.apply(pkt, hdr);
        transition accept;
    }
}
...
control SwitchEgressDeparser(
    ...
    apply {
        ...
        if (eg_dprsr_md.mirror_type == MIRROR_TYPE_E2E) {
            mirror.emit<mirror_h>(eg_md.egr_mir_ses, {eg_md.mirror_header.pkt_type
                                                      });
        }
...
control SwitchEgress(
    ...
    Register<bit<32>, _>(1, 0) epoch;
    RegisterAction<bit<32>, _, bit<32>>(epoch) epoch_update_action = {
        void apply(inout bit<32> val, out bit<32> rv) {
            //1000000代表1ms 
            if ((bit<32>)eg_prsr_md.global_tstamp - val >= 1000000) {
                rv = 1;
    ...
    action epoch_update() {
        eg_md.epoch_end_flag = (bit<1>)epoch_update_action.execute(0);
        // eg_md.epoch_end_flag = (bit<1>)epoch_update_action.execute(1);
        // eg_md.epoch_end_flag = 0;
    }
    ...
    */
    ...
    action all_rd_and_cl() {
        eg_md.mediumIBLT_op = 2;
        eg_md.highIBLT_op = 2;
        eg_md.medium_row0_index = (bit<10>)eg_md.rdclIBLT_index;
        eg_md.medium_row1_index = (bit<10>)eg_md.rdclIBLT_index;
        eg_md.high_row0_index = (bit<10>)eg_md.rdclIBLT_index;
        eg_md.high_row1_index = (bit<10>)eg_md.rdclIBLT_index;
        hdr.bridged_md.is_5tup_pkt = 1;
    ...
    table tb_operation_decide {
        ...
        const entries = {
            ...
            (0, 0..1024) : all_rd_and_cl();
            //中拥塞包，一律插入中IBLT
            (1, 0..0xFFFFF) : mediumIBLT_insert();
            //高拥塞包，一律插入高IBLT
            (2, 0..0xFFFFF) : highIBLT_insert();
        }
    ...
    */
    ...
    //取消镜像头，防止egress拥塞
    action cancel_mirror() {
        hdr.ipv4.protocol = hdr.telemetry.protocol;
        hdr.ipv4.total_len = hdr.ipv4.total_len - 70;
    ...
    action set_mirror(MirrorId_t egr_ses) {
        ...
        eg_md.mirror_header.pkt_type = PKT_TYPE_MIRROR;
    }

    //镜像表，通过flowCnt决定是否镜像
    // table tb_mirror_decide {
    //     key = {
    //         hdr.telemetry.fCnt_0_0 : exact;
    //         hdr.telemetry.fCnt_0_1 : exact;
    //         hdr.telemetry.fCnt_1_0 : exact;
    //         hdr.telemetry.fCnt_1_1 : exact;
    //     }
    ...
    //     default_action = set_mirror(33);

    //     const entries = {
    //         //全0，代表遍历的当前行无数据，不镜像
    //         (0, 0, 0, 0) : cancel_mirror();
    //     }
    // }


    apply {
        // if (eg_md.mirror_header.pkt_type == PKT_TYPE_MIRROR) {
        // }
        // else {
            //分类低中高三种拥塞
            //0:低拥塞，1:中拥塞，2:高拥塞
            ...
            //     //set_mirror(33);
            //     //不为0
            //     if (hdr.telemetry.fCnt_0_0 != 0 || hdr.telemetry.fCnt_0_1 != 0 || hdr.telemetry.fCnt_1_0 != 0 || hdr.telemetry.fCnt_1_1 != 0) {
            //         set_mirror(33);
            //     }
            //     //1023末尾
            //     else if (eg_md.high_row0_index == 1023 || eg_md.medium_row0_index == 1023) {
            //         set_mirror(33);
            //     }
            //     else {
            //         cancel_mirror();
